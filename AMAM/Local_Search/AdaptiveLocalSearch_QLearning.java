...
public class AdaptiveLocalSearch_QLearning extends Method{
	...
	public Solution runMethod(Problem p, Solution so, int id_agent) {
		...
		int states[] = new int[this.getParameters().getMethodParametersI(id_agent).getQLearning().getQSize()];
		int states_count;
		...
		while (improved) {
			...
			states_count = 0;
			for(int i = 0; i < this.getParameters().getMethodParametersI(id_agent).getQLearning().getQSize(); i++) {
				states[i] = i;
			}
			
			//random initial state
			next_state = this.getParameters().getMethodParametersI(id_agent).getQLearning().chooseAnAction(0, 2); //2 --> random function
			//System.out.println("\nState: " + next_state);
			...
			while (episode) {
				...
				if(no_improvement == 0) {
					state = next_state;
					//Reinforcement Learning
					next_state = this.getParameters().getMethodParametersI(id_agent).getQLearning().chooseAnAction(state, 1);  // 1 --> epsilon greedy function
				}
				else {  // if not improved in the first --> random function
					//Reinforcement Learning
					next_state = this.getParameters().getMethodParametersI(id_agent).getQLearning().chooseAnAction(state, 2);  // 2 --> random function
				}
				...
				if(criteria.acceptationCriteria(this.s_line, this.best_solution, this.element, p)) {
					...
					if((no_improvement > 10) || (states_count == this.getParameters().getMethodParametersI(id_agent).getQLearning().getQSize())) {
						improved = false;
						episode = false;
					}
					//System.out.println("Não melhorou a solução!");
			...
			episode = true;
			//this.getParameters().getMethodParametersI(id_agent).getQLearning().showQMatrix();
